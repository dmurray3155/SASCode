...
%macro SetFylName(SubTyp, Subj, Grd, IABShortTitle, _AcadYear_, _vrsn_);
	...
	%if &TSubTyp.=ICA %then %do;
		%let _FylName_=SBAC-&TSubTyp.-COMBINED-&Subj.-&Grd.-&_AcadYear_._v&_vrsn_..xlsx;
	%end;
	%else %if &TSubTyp.=IAB or &TSubTyp.=FIAB %then %do;
		%let _FylName_=SBAC-&TSubTyp.-&Subj.-&IABShortTitle.-&Grd.-&_AcadYear_._v&_vrsn_..xlsx;
	%end;
...
%macro Cre8TstSegNames(TSubTyp, tdfvrsn, Subj, Grd, IABShortTitle, _AcadYear_, ReptResults);
	...
	%if &NumTtlSegs=1 %then %do;
		...
		%end;
		%let _SegmentId_=SBAC-&TSubTyp.-&Subj.-&IABShortTitle.-&Grd.;
		%let _SegmentLabel_=Grade &Grd. &Subj. - &FTSg1. (&TSubTyp.);
		%if &TSubTyp.=Practice %then %do;
		...
		%end;
		%let _SegmentFormId_=Grade &Grd. &Subj. - &FTSg1. (&TSubTyp.);
		%if &TSubTyp.=Practice %then %do;
	...
	%if &NumTtlSegs=2 %then %do;	/* These are IABs with one test name and two segments	*/
		%if &TSubTyp.=IAB or &TSubTyp.=FIAB or &TSubTyp.=Practice or &TSubTyp.=Training %then %do;
			%let _TestId_=SBAC-IAB-&Subj.-&IABShortTitle.-&Grd.;
			%let _TestLabel_=Grade &Grd. &Subj. - &FTSg1. (&TSubTyp.);
			%if &TSubTyp.=Practice %then %do;
				%tranwrdMac(_TestLabel_, (Practice), );
			%end;
			...
			%if &Grd.=11 %then %do;
				%tranwrdMac(_TestLabel_, Grade 11, High School);
			%end;
			%let _SegmentId_=SBAC-IAB-&Subj.-&IABShortTitle.-&Grd.-SEG1-&TSeg1.;
			%let _SegmentLabel_=Grade &Grd. &Subj. - &FTSg1. &TSeg1. (&TSubTyp.);
			%if &TSubTyp.=Practice %then %do;
				%tranwrdMac(_SegmentLabel_, (Practice), );
			%end;
			%if &TSubTyp.=Training %then %do;
			...
			%if &Grd.=11 %then %do;
				%tranwrdMac(_SegmentLabel_, Grade 11, High School);
			%end;
			%let _SegmentFormId_=Grade &Grd. &Subj. - &FTSg1. &TSeg1. (&TSubTyp.);
			%if &TSubTyp.=Practice %then %do;
				%tranwrdMac(_SegmentFormId_, (Practice), );
			%end;
			%if &TSubTyp.=Training %then %do;
			...
			%if &Grd.=11 %then %do;
				%tranwrdMac(_SegmentFormId_, Grade 11, High School);
			%end;
			%let _SegmentIdB_=SBAC-IAB-&Subj.-&IABShortTitle.-&Grd.-SEG2-&TSeg2.;
			%let _SegmentLabelB_=Grade &Grd. &Subj. - &FTSg1. &TSeg2. (&TSubTyp.);
			%if &TSubTyp.=Practice %then %do;
	...
	/*	%if &TSubTyp.=Practice %then %do;
			...
			%end;	*/
			%let _SegmentFormIdB_=Grade &Grd. &Subj. - &FTSg1. &TSeg2. (&TSubTyp.);
			%if &TSubTyp.=Practice %then %do;
				%tranwrdMac(_SegmentFormIdB_, (Practice), );
			%end;
			%if &TSubTyp.=Training %then %do;
		...
		%else %if &TSubTyp.=ICA %then %do;
			%let _TestId_=SBAC-&TSubTyp.-&Subj.-&STSg1.-&Grd.;
			%let _TestLabel_=Grade &Grd. &Subj. - &FTSg1. (&TSubTyp.);
			%let _TestId2_=SBAC-&TSubTyp.-&Subj.-&STSg2.-&Grd.;
			%let _TestLabel2_=Grade &Grd. &Subj. - &FTSg2. (&TSubTyp.);
			%if &Subj.=MATH and %eval(&Grd. < 6) %then %do;
				%let _SegmentId_=SBAC-&TSubTyp.-&Subj.-&STSg1.-&Grd.;
				%let _SegmentLabel_=Grade &Grd. &Subj. - &FTSg1. (&TSubTyp.);
				%let _SegmentFormId_=Grade &Grd. &Subj. - &FTSg1 (&TSubTyp.);
				%let _SegmentId2_=SBAC-&TSubTyp.-&Subj.-&STSg2.-&Grd.;
				%let _SegmentLabel2_=Grade &Grd. &Subj. - &FTSg2. (&TSubTyp.);
				%let _SegmentFormId2_=Grade &Grd. &Subj. - &FTSg2. (&TSubTyp.);
			%end;
			%else %do;
				%let _SegmentId_=SBAC-&TSubTyp.-&Subj.-&STSg1.-&Grd.-SEG1-&TSeg1.;
				%let _SegmentLabel_=Grade &Grd. &Subj. - &FTSg1. &TSeg1. (&TSubTyp.);
				%let _SegmentFormId_=Grade &Grd. &Subj. - &FTSg1. &TSeg1. (&TSubTyp.);
				%let _SegmentIdB_=SBAC-&TSubTyp.-&Subj.-&STSg2.-&Grd.-SEG2-&TSeg2.;
				%let _SegmentLabelB_=Grade &Grd. &Subj. - &FTSg1. &TSeg2. (&TSubTyp.);
				%let _SegmentFormIdB_=Grade &Grd. &Subj. - &FTSg1. &TSeg2. (&TSubTyp.);
			%end;
		%end;
	...
	%if &NumTtlSegs=3 %then %do;
		%if "&STSg2."="&STSg1." %then %do;	/* This is like a math ICA - like grade 8	*/
			...
			%let _SegmentFormId_=Grade &Grd. &Subj. - &FTSg1. - &TSeg1. (&TSubTyp.);
			%let _SegmentIdB_=SBAC-&TSubTyp.-&Subj.-&STSg2.-&Grd.-SEG2-&TSeg2.;
			%let _SegmentLabelB_=Grade &Grd. &Subj. - &FTSg2. - &TSeg2. (&TSubTyp.);
			%let _SegmentFormIdB_=Grade &Grd. &Subj. - &FTSg2. - &TSeg2. (&TSubTyp.);
			%let _TestId2_=SBAC-&TSubTyp.-&Subj.-&STSg3.-&Grd.;
			%let _TestLabel2_=Grade &Grd. &Subj. - &FTSg3. (&TSubTyp.);
			%let _SegmentId2_=SBAC-&TSubTyp.-&Subj.-&StSg3.-&Grd.;
			%let _SegmentLabel2_=Grade &Grd. &Subj. - &FTSg3. (&TSubTyp.);
			%let _SegmentFormId2_=Grade &Grd. &Subj. - &FTSg3. (&TSubTyp.);
		%end;
		%if "&STSg3."="&STSg2." %then %do;	/*	This is like an ELA ICA - like grade 11	*/
			...
			%let _TestId2_=SBAC-&TSubTyp.-&Subj.-&STSg2.-&Grd.;
			%let _TestLabel2_=Grade &Grd. &Subj. - &FTSg2. (&TSubTyp.);
			%let _SegmentId2_=SBAC-&TSubTyp.-&Subj.-&STSg2.-&Grd.-SEG1-&TSeg2.;
			%let _SegmentLabel2_=Grade &Grd. &Subj. - &FTSg2. - &TSeg2. (&TSubTyp.);
			%let _SegmentFormId2_=Grade &Grd. &Subj. - &FTSg2. - &TSeg2. (&TSubTyp.);
			%let _SegmentId2B_=SBAC-&TSubTyp.-&Subj.-&StSg3.-&Grd.-SEG2-&TSeg3.;
			%let _SegmentLabel2B_=Grade &Grd. &Subj. - &FTSg3. - &TSeg3. (&TSubTyp.);
			%let _SegmentFormId2B_=Grade &Grd. &Subj. - &FTSg3. - &TSeg3. (&TSubTyp.);
		%end;
	%end;
	...
	%if &TSubTyp.=ICA %then %do;
		%let _PackageId_=SBAC-&TSubTyp.-COMBINED-&Subj.-&Grd.;
	%end;
	%else %if &TSubTyp.=IAB or &TSubTyp.=FIAB or &TSubTyp.=Practice %then %do;
		%let _PackageId_=SBAC-&TSubTyp.-&Subj.-&IABShortTitle.-&Grd.;
	%end;
	%else %if &TSubTyp.=Training %then %do;
...
%macro GenFIABNames;
	%Cre8TstSegNames(FIAB, 01, ELA, 3, ResearchAnalyzeInfo, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, ELA, 3, ResearchInterpInteg, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, ELA, 3, WriteNarrative, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, ELA, 4, ResearchAnalyzeInfo, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, ELA, 4, ResearchInterpInteg, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, ELA, 4, WriteNarrative, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, ELA, 5, ResearchAnalyzeInfo, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, ELA, 5, ResearchInterpInteg, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, ELA, 5, WriteNarrative, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, ELA, 6, ResearchAnalyzeInfo, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, ELA, 6, ResearchInterpInteg, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, ELA, 6, WriteNarrative, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, ELA, 7, ResearchAnalyzeInfo, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, ELA, 7, ResearchInterpInteg, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, ELA, 7, WriteNarrative, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, ELA, 8, ResearchAnalyzeInfo, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, ELA, 8, ResearchInterpInteg, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, ELA, 8, WriteNarrative, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, ELA, 11, ResearchAnalyzeInfo, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, ELA, 11, ResearchInterpInteg, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, ELA, 11, WriteNarrative, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, MATH, 3, TAOA, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, MATH, 3, TBOA, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, MATH, 3, TCOA, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, MATH, 4, TAOA, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, MATH, 4, TFNF, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, MATH, 4, THNF, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, MATH, 5, TAOA, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, MATH, 5, TDNBT, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, MATH, 5, TENF, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, MATH, 6, TBNS, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, MATH, 6, TFEE, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, MATH, 6, TGEE, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, MATH, 7, TCEE, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, MATH, 7, TDEE, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, MATH, 7, TEG, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, MATH, 8, TCEE, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, MATH, 8, TDEE, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, MATH, 8, TGG, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, MATH, 11, THREI, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, MATH, 11, TIREILinExp, 2019-2020);
	%Cre8TstSegNames(FIAB, 01, MATH, 11, TIREIQuad, 2019-2020);
%mend GenFIABNames;
%*	%GenFIABNames;

%macro GenIABNames;
	%Cre8TstSegNames(IAB, 02, ELA, 3, BriefWrites, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 3, Editing, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 3, LangVocab, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 3, ListenInterpet, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 3, Perf-Opinion-Beetles, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 3, ReadInfo, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 3, ReadLit, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 3, Research, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 3, Revision, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 4, BriefWrites, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 4, Editing, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 4, LangVocab, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 4, ListenInterpet, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 4, Perf-Narrative-UnlikelyAnimal, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 4, ReadInfo, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 4, ReadLit, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 4, Research, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 4, Revision, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 5, BriefWrites, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 5, Editing, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 5, LangVocab, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 5, ListenInterpet, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 5, Perf-Narrative-Whales, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 5, ReadInfo, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 5, ReadLit, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 5, Research, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 5, Revision, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 6, BriefWrites, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 6, Editing, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 6, LangVocab, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 6, ListenInterpet, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 6, Perf-Argument-Multivitamins, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 6, ReadInfo, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 6, ReadLit, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 6, Research, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 6, Revision, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 7, BriefWrites, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 7, Editing, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 7, LangVocab, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 7, ListenInterpet, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 7, Perf-Explanatory-MobileEdTech, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 7, ReadInfo, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 7, ReadLit, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 7, Research, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 7, Revision, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 8, BriefWrites, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 8, EditRevise, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 8, ListenInterpet, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 8, Perf-Explanatory-CompareAncient, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 8, ReadInfo, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 8, ReadLit, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 8, Research, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 11, BriefWrites, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 11, Editing, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 11, LangVocab, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 11, ListenInterpet, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 11, Perf-Explanatory-Marshmallow, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 11, ReadInfo, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 11, ReadLit, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 11, Research, 2019-2020);
	%Cre8TstSegNames(IAB, 02, ELA, 11, Revision, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 3, G, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 3, MD, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 3, NBT, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 3, NF, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 3, OA, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 3, Perf-OrderForm, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 4, G, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 4, MD, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 4, NBT, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 4, NF, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 4, OA, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 4, Perf-AnimalJumping, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 5, G, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 5, MD, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 5, NBT, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 5, NF, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 5, OA, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 5, Perf-TurtleHabitat, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 6, EE, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 6, G, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 6, NS, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 6, Perf-CellPhonePlan, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 6, RP, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 6, SP, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 7, EE, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 7, G, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 7, NS, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 7, Perf-CampingTasks, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 7, RP, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 7, SP, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 8, EE, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 8, EE2, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 8, F, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 8, G, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 8, NS, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 8, Perf-BaseballTickets, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 11, AlgLin, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 11, AlgQuad, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 11, GCO, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 11, GMD, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 11, GeoRightTriRatios, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 11, IF, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 11, NQ, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 11, Perf-TeenDrivingRest, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 11, SP, 2019-2020);
	%Cre8TstSegNames(IAB, 02, MATH, 11, SSE, 2019-2020);
%mend GenIABNames;
%*	%GenIABNames;

%macro GenICANames;
	%Cre8TstSegNames(ICA, 02, ELA, 3, FIXED, 2019-2020);
	%Cre8TstSegNames(ICA, 02, ELA, 4, FIXED, 2019-2020);
	%Cre8TstSegNames(ICA, 02, ELA, 5, FIXED, 2019-2020);
	%Cre8TstSegNames(ICA, 02, ELA, 6, FIXED, 2019-2020);
	%Cre8TstSegNames(ICA, 02, ELA, 7, FIXED, 2019-2020);
	%Cre8TstSegNames(ICA, 02, ELA, 8, FIXED, 2019-2020);
	%Cre8TstSegNames(ICA, 02, ELA, 11, FIXED, 2019-2020);
	%Cre8TstSegNames(ICA, 02, MATH, 3, FIXED, 2019-2020);
	%Cre8TstSegNames(ICA, 02, MATH, 4, FIXED, 2019-2020);
	%Cre8TstSegNames(ICA, 02, MATH, 5, FIXED, 2019-2020);
	%Cre8TstSegNames(ICA, 02, MATH, 6, FIXED, 2019-2020);
	%Cre8TstSegNames(ICA, 02, MATH, 7, FIXED, 2019-2020);
	%Cre8TstSegNames(ICA, 02, MATH, 8, FIXED, 2019-2020);
	%Cre8TstSegNames(ICA, 02, MATH, 11, FIXED, 2019-2020);
%mend GenICANames;
%*	%GenICANames;
...
/*	libname XLOut XLSX "&UniRoot./WrkHere./OutFiles/&_FylName_..xlsx";	*/
	...
	proc sql noprint;
		...
		where upcase(Subject) = "&Subj." and grade = &Grd.;
	quit;
	data PackageData;
		...
		if InputParam='_Subj_' then InputParam="&Subj.";
	...
	%if &TSubTyp=IAB or &TSubTyp=FIAB %then %let _lastCol_=5;
	%else %if &TSubTyp=ICA %then %do;
		%if &Subj.=ELA %then %let _lastCol_=32;
		%else %if &Subj.=MATH %then %let _lastCol_=26;
	%end;
	%if &TSubTyp.=Practice or &TSubTyp.=Training %then %do;
		data ScoringData;
			set SrcData.ScoringTemplate;
	...
	%else %do;
		proc sql;
			...
			%end;
			%else %do;
				where TSubType = "&TSubTyp."
			%end;
			%if &TSubTyp.=ICA %then %do;
				and upcase(Subject) = "&Subj."
			%end;
			;
	...
	data SegmentsData;
		...
		%if &TSubTyp.=ICA %then %do;
			...
			%end;
		%end;
		%else %if &TSubTyp.=IAB or &TSubTyp=FIAB or &TSubTyp=Practice or &TSubTyp=Training %then %do;
	...
	data TDF_DataSF;
		...
		%if &TSubTyp.=IAB or &TSubTyp.=FIAB or &TSubTyp.=Practice or &TSubTyp.=Training %then %do;
			...
			else if SegmentPosition = 2 then do;
				SegId = "&_SegmentIdB_.";
...
%macro WrytPSLFylsAndBAT(TSubTyp, tdfvrsn, Subj, Grd, IABShortTitle, _FylName_);
	...
	%if &TSubTyp.=IAB or &TSubTyp.=FIAB or &TSubTyp.=Practice or &TSubTyp.=Training %then %let FldRng=E1:I1;
	%else %if &TSubTyp.=ICA %then %do;
		%if &Subj.=ELA %then %let FldRng=E1:AJ1;
		%else %if &Subj.=MATH %then %let FldRng=E1:AD1;
	%end;
	data _null_;
	...
	%mend SegmentsPage;
	%macro ExecSP;
		%if &Subj. = MATH and %eval(&Grd. > 5) %then %do;
			%SegmentsPage(E1:F1);
			%let ExecSegMrg = yes;
		%end;
		%else %if &Subj. = ELA and &TSubTyp. = ICA %then %do;
			%SegmentsPage(F1:G1);
			%let ExecSegMrg = yes;
		%end;
		%else %if &Subj. = ELA and &TSubTyp. = IAB and %eval(&Grd. > 5) %then %do;
			%SegmentsPage(E1:F1);
			%let ExecSegMrg = yes;
		%end;
		%else %if &TSubTyp.=Practice and &Subj.=ELA and "&IABShortTitle."="PRAC-Perf" %then %do;
			%SegmentsPage(E1:F1);
			%let ExecSegMrg = yes;
		%end;
		%else %if &TSubTyp.=Training and &Subj.=MATH and %eval(&Grd. > 5) %then %do;
			%SegmentsPage(E1:F1);
			%let ExecSegMrg = yes;
		%end;
		%else %do;
