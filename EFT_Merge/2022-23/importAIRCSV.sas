/*==================================================================================================*
 ...
 |	2024 02 17		Copied from 2018-19 project for use here for 2022-23.															|
 *==================================================================================================*/
...
%macro ImprtAIR(CntGrd, TargetFyl);
	...
	%SetDSLabel;
	data bigFast.&CntGrd._AIRSrc (compress=yes label="&DSLabel.");
		...
		format TestID $75. TestSubject $5. TestedGrade $2. vndr_test_event_ID 13.0
			tdsopportunityguid $36. AssessmentStartDateTime $12. AssessmentEndDateTime $12.
			COMPONENT_1_OPPKEY $36. COMPONENT_2_OPPKEY $36.
			...
			ELL $3. overall_thetascore 18.14 ScaledScore 4.0 EBSR_Part1 $312. EBSR_Part2 $312.;
		%&CntGrd._Fmt;
		input TestID $ TestSubject $ TestedGrade $  vndr_test_event_ID
			tdsopportunityguid $ AssessmentStartDateTime $ AssessmentEndDateTime $
			COMPONENT_1_OPPKEY $ COMPONENT_2_OPPKEY $
			COMPONENT_1_OPPID COMPONENT_2_OPPID transformed_clientname $
			EconomicDisadvantaged $ Ethnicity gender $ PrimaryDisability $
			ELL $ overall_thetascore ScaledScore
			%do fld = 20 %to &LastField.;
				%if %substr(%trim(&&field&fld..), 3, 2) = RV %then %do;
					%trim(&&field&fld.._str) $ 
				%end;
				...
				if substr(&&field&fld.._str, 1, 17) = 'choiceInteraction' then do;
					...
					select(fieldLen);
						...
						when(231) &&field&fld.. = compress(substr(&&field&fld.._str, 28, 1)||'|'||substr(&&field&fld.._str, 57, 1)||'|'||
											substr(&&field&fld.._str, 86, 1)||'|'||substr(&&field&fld.._str, 115, 1)||'|'||substr(&&field&fld.._str, 144, 1)||'|'||
											substr(&&field&fld.._str, 173, 1)||'|'||substr(&&field&fld.._str, 202, 1)||'|'||substr(&&field&fld.._str, 231, 1));
						otherwise do;
							&&field&fld.. = "PROB";
					...
					%do prt = 1 %to 2;
						...
						select(fieldLen);
							...
							when(238) EBSR_Part&prt. = compress(substr(EBSR_Part&prt._str, 61, 1)||'|'||substr(EBSR_Part&prt._str, 90, 1)||'|'||substr(EBSR_Part&prt._str, 119, 1)||'|'||
												substr(EBSR_Part&prt._str, 148, 1)||'|'||substr(EBSR_Part&prt._str, 177, 1)||'|'||substr(EBSR_Part&prt._str, 206, 1)||'|'||substr(EBSR_Part&prt._str, 235, 1));
							otherwise do;
...
/*	If you're looking for a macro call for ela11 then look in the SAS code file named importAIRCSV_ela11.sas	*/
...
/*	If you're looking for a macro call for math11 then look in the SAS code file named importAIRCSV_math11.sas	*/
